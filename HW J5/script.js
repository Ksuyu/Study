...
function printBill(billArray) {

  let str = '';
  for (let i = 0; i < billArray.length; i++) {
  str += `------
     Назва: ${ billArray[i].name }
      Ціна: ${ billArray[i].price } грн.
    Кіл-ть: ${ billArray[i].qty } шт.
  Вартість: ${ billArray[i].price * billArray[i].qty } грн.
  `
  }
...
console.log(printBill(billArray));
...
function calcTotalAmount(billArray) {

  let total = 0;
  for (let i = 0; i < billArray.length; i++) {
    total += billArray[i].price * billArray[i].qty;
  }
...
console.log('Загальна сума покупки: ' + calcTotalAmount(billArray) + ' грн.');
...
function showMostExpensiveItem(billArray) {

  let max = 0;
        
  for (let i = 0; i < billArray.length; i++) {
      if (max < billArray[i].price * billArray[i].qty ) {
          max = billArray[i].price * billArray[i].qty;
      }
    }
...
console.log('Найдорожча покупка в чеку: ' + showMostExpensiveItem(billArray) + ' грн.');


// Підрахунок середньої вартості одного товару у чеку.

function calcAveragePrice(billArray) {

    averagePrice = calcTotalAmount(billArray) / billArray.length;

    return averagePrice.toFixed(2);

}

console.log('Cередня вартість одного товару у чеку: ' + calcAveragePrice(billArray) + ' грн.');
